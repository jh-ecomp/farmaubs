...
import type {
  IAuthRepository,
  UserAuthRecord,
} from "../../src/modules/auth/domain/ports/auth.repository.port";
import { AUTH_REPOSITORY } from "../../src/modules/auth/domain/ports/auth.repository.port";
import { LoginUseCase } from "../../src/modules/auth/domain/use-cases/login.use-case";
...
function makeRepo(
  partial: Partial<IAuthRepository> = {},
): jest.Mocked<IAuthRepository> {
  return {
...
function makeUsuario(overrides: Partial<UserAuthRecord> = {}): UserAuthRecord {
  return {
    id: "uuid-usuario",
