...
export class LoginUseCase {
  ...
  async executar(
    ...
    const usuario = await this.authRepo.buscarUsuarioPorEmail(normalizado);
    ...
    if (!senhaValida) {
      await this.authRepo.registrarFalhaLogin(usuario.id);
      return { ok: false, motivo: "CREDENCIAIS_INVALIDAS" };
    }

    await this.authRepo.resetarEstadoLogin(usuario.id);

    const expiraEm = new Date(Date.now() + this.ttlMinutos * 60_000);

    const token = await this.authRepo.criarSessao({
